...
job_type :rake,    "cd :path && rvm use ruby-2.1.5@gigi && :environment_variable=:environment :bundle_command rake :task --silent :output"
job_type :script,  "cd :path && rvm use ruby-2.1.5@gigi && :environment_variable=:environment :bundle_command script/:task :output"
